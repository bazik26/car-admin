...
export class SoundService {
  ...
  toggleSound(): boolean {
    ...
    localStorage.setItem('admin_chat_sound_enabled', this.isEnabled.toString());
    return this.isEnabled;
  }

  isSoundEnabled(): boolean {
    const saved = localStorage.getItem('admin_chat_sound_enabled');
    if (saved !== null) {
  ...
  // Звук нового сообщения от клиента
  playNewMessageSound() {
  ...
  // Звук отправки сообщения админом
  playSendSound() {
    if (!this.isEnabled || !this.audioContext) return;
